...
export class LoginComponent implements OnInit {
  ...
  login() {
    ...
    this.authService.authUsuario(objLogin).subscribe(
      (usuarioLogado) => {
        ...
        if (usuarioLogado.data.active && usuarioLogado.data.tipo != '2') {
          ...
          usuarioLogado.data.active &&
          usuarioLogado.data.tipo == '2'
        ) {
          this.router.navigate(['admin']);
        } else {
          this.suporte.showMessageErro(
            'OPS sua conta esta inativa entre contato!'
          );
          localStorage.clear();
        }
      },
